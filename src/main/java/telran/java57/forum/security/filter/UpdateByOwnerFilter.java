...
public class UpdateByOwnerFilter implements Filter {
    ...
    private boolean checkEndPoint(String method, String path) {
        return HttpMethod.PUT.matches(method) && path.matches("/account/user/[A-Za-z0-9@+.]+")
                || HttpMethod.POST.matches(method) && path.matches("/forum/post/[A-Za-z0-9@+.]+")
                || HttpMethod.PUT.matches(method) && path.matches("/forum/post/\\w+/comment/[A-Za-z0-9@+.]+");
    }
}
